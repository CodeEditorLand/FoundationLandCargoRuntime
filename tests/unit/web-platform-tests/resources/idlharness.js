...
export default function (self) {
	(() => {
		...
		if (!("subsetTestByKey" in self)) {
			self.subsetTestByKey = (key, callback, ...args) =>
				callback(...args);
		...
		function constValue(cnt) {
			...
			if (cnt.type === "NaN") return Number.NaN;
			if (cnt.type === "Infinity")
				return cnt.negative
		...
		function minOverloadLength(overloads) {
			...
			return overloads
				.map((attr) =>
					attr.arguments
						? attr.arguments.filter(
								(arg) => !arg.optional && !arg.variadic,
							).length
						: 0,
				)
				.reduce((m, n) => Math.min(m, n));
		}
		...
		function throwOrReject(a_test, operation, fn, obj, args, message, cb) {
			if (operation.idlType.generic !== "Promise") {
				...
				try {
					...
					a_test.step(() => {
						assert_unreached(
							'Throws "' + e + '" instead of rejecting promise',
						);
						cb();
					});
				}
		...
		IdlArray.prototype.mark_as_untested = (parsed_idls) => {
		...
		IdlArray.prototype.is_excluded_by_options = (name, options) =>
			options &&
		...
		IdlArray.prototype.internal_add_idls = function (parsed_idls, options) {
			...
			parsed_idls.forEach(
				function (parsed_idl) {
					...
					if (
						parsed_idl.partial &&
						partial_types.includes(parsed_idl.type)
					) {
					...
					switch (parsed_idl.type) {
						...
						case "dictionary":
							...
							// checks
							this.members[parsed_idl.name] = new IdlDictionary(
								parsed_idl,
							);
							break;

						case "typedef":
							this.members[parsed_idl.name] = new IdlTypedef(
								parsed_idl,
							);
							break;

						case "callback":
							this.members[parsed_idl.name] = new IdlCallback(
								parsed_idl,
							);
							break;

						case "enum":
							this.members[parsed_idl.name] = new IdlEnum(
								parsed_idl,
							);
							break;
						...
						case "namespace":
							this.members[parsed_idl.name] = new IdlNamespace(
								parsed_idl,
							);
							break;
					...
					if (thing instanceof IdlInterface) {
						...
						while (thing) {
							...
							if (mixins) {
								...
								if (
									mixins.some((m) =>
										m.has_to_json_regular_operation(),
									)
								) {
									return true;
								}
		...
		function exposure_set(object, default_set) {
			var exposed =
				object.extAttrs &&
				object.extAttrs.filter((a) => a.name === "Exposed");
		...
		IdlArray.prototype.test = function () {
			...
			for (const member of Object.values(this.members).filter(
				(m) => m.base,
			)) {
				...
				const lhs_is_interface =
					this.members[lhs] instanceof IdlInterface;
				const rhs_is_interface =
					...
					objects.forEach(
						function (str) {
							if (
								...
								throw new IdlHarnessError(
									`Invalid object member name ${name}`,
								);
							}
			...
			this.partials.forEach(
				function (parsed_idl) {
					const originalExists =
						...
						(this.members[parsed_idl.name] instanceof
							IdlInterface ||
							this.members[parsed_idl.name] instanceof
								IdlDictionary ||
							this.members[parsed_idl.name] instanceof
					...
					if (parsed_idl.extAttrs) {
						...
						parsed_idl.extAttrs.forEach(
							function (extAttr) {
								...
								this.members[parsed_idl.name].extAttrs.push(
									extAttr,
								);
							}.bind(this),
						);
								...
								var clash = parsed_idl.members.find(
									function (member) {
										return this.members[
											...
											function (m) {
												return this.are_duplicate_members(
													m,
													member,
												);
											}.bind(this),
		...
		IdlArray.prototype.are_duplicate_members = (m1, m2) => {
			if (m1.name !== m2.name) {
				return false;
			}
			if (
		...
		IdlArray.prototype.assert_type_is = function (value, type) {
			...
			if (
				type.generic === "sequence" ||
				type.generic == "ObservableArray"
			) {
			...
			switch (type) {
				...
				case "long":
					...
					assert_true(
						...
						"long " +
							value +
							" should be in range [-2147483648, 2147483647]",
					);
					return;
				...
				case "unsigned long":
					...
					assert_true(
						...
						"unsigned long " +
							value +
							" should be in range [0, 4294967295]",
					);
					return;
				...
				case "DOMTimeStamp":
					...
					assert_true(
						value >= 0,
						"unsigned long long should be positive",
					);
					return;
				...
				case "float":
					...
					assert_not_equals(value, Number.POSITIVE_INFINITY);
					assert_not_equals(value, Number.NEGATIVE_INFINITY);
					assert_not_equals(value, Number.NaN);
					return;
			...
			if (this.members[type] instanceof IdlInterface) {
				...
				throw new IdlHarnessError(
					"Type " +
						type +
						" isn't an interface, callback or dictionary",
				);
			}
		};
		...
		IdlObject.prototype.has_extended_attribute = function (name) {
			 ...
			 */
			return this.extAttrs.some((o) => o.name == name);
		};
		...
		IdlDictionary.prototype.get_reverse_inheritance_stack = function () {
			return IdlInterface.prototype.get_reverse_inheritance_stack.call(
				this,
			);
		};
		...
		IdlInterface.prototype.has_constants = function () {
			return this.members.some((member) => member.type === "const");
		};

		IdlInterface.prototype.get_unscopables = function () {
			return this.members.filter((member) => member.isUnscopable);
		};

		IdlInterface.prototype.is_global = function () {
			return this.extAttrs.some(
				(attribute) => attribute.name === "Global",
			);
		};
		...
		IdlInterface.prototype.get_legacy_namespace = function () {
			var legacyNamespace = this.extAttrs.find(
				(attribute) => attribute.name === "LegacyNamespace",
			);
		...
		IdlInterface.prototype.has_to_json_regular_operation = function () {
			return this.members.some((m) => m.is_to_json_regular_operation());
		};

		IdlInterface.prototype.has_default_to_json_regular_operation =
			function () {
		...
		IdlInterface.prototype.constructors = function () {
			return this.members.filter((m) => m.type == "constructor");
		};
		...
		IdlInterface.prototype.test_self = function () {
			...
			if (this.has_extended_attribute("LegacyWindowAlias")) {
				subsetTestByKey(
					...
					test,
					function () {
						var aliasAttrs = this.extAttrs.filter(
							(o) => o.name === "LegacyWindowAlias",
						);
			...
			if (this.has_extended_attribute("LegacyFactoryFunction")) {
				var constructors = this.extAttrs.filter(
					(attr) => attr.name == "LegacyFactoryFunction",
				);
				...
				subsetTestByKey(
					...
					(() => {
						...
						assert_false(
							"get" in desc,
							name + " should not have a getter",
						);
						assert_false(
							"set" in desc,
							name + " should not have a setter",
						);
					}).bind(this),
				...
				subsetTestByKey(
					...
					function () {
						...
						assert_false(
							desc.enumerable,
							"prototype should not be enumerable",
						);
						assert_false(
							desc.configurable,
						...
						assert_false(
							"get" in desc,
							"prototype should not have a getter",
						);
						assert_false(
							"set" in desc,
							"prototype should not have a setter",
						);
					}.bind(this),
					this.name +
						" interface: named constructor prototype property",
				);
				...
				subsetTestByKey(
					...
					(() => {
						...
						assert_true(
							desc.configurable,
							"length should be configurable",
						);
						assert_false(
							"get" in desc,
							"length should not have a getter",
						);
						assert_false(
							"set" in desc,
							"length should not have a setter",
						);
					}).bind(this),
				...
				subsetTestByKey(
					...
					function () {
						...
						var name = constructor.rhs.value;
						var args = constructor.arguments.map((arg) =>
							create_suitable_object(arg.idlType),
						);
						assert_throws_js(
				...
				function () {
					...
					if (this.is_callback()) {
						assert_false(
							...
							this.name +
								' should not have a "prototype" property',
						);
						return;
					}
			...
			subsetTestByKey(
				...
				function () {
					...
					assert_own_property(
						...
						"constructor",
						this.name +
							'.prototype does not have own property "constructor"',
					);
					var desc = Object.getOwnPropertyDescriptor(
						this.get_interface_object().prototype,
						"constructor",
					...
					assert_false(
						"get" in desc,
						this.name +
							".prototype.constructor should not have a getter",
					);
					assert_false(
						"set" in desc,
						this.name +
							".prototype.constructor should not have a setter",
					);
					assert_true(
						desc.writable,
					...
					assert_false(
						desc.enumerable,
						this.name +
							".prototype.constructor should not be enumerable",
					);
					assert_true(
						desc.configurable,
						this.name +
							".prototype.constructor should be configurable",
					);
					assert_equals(
				...
				function () {
					...
					if (unscopables.length != 0) {
						assert_own_property(
							...
							Symbol.unscopables,
							this.name +
								".prototype should have an @@unscopables property",
						);
						var desc = Object.getOwnPropertyDescriptor(
							proto,
							Symbol.unscopables,
						...
						assert_equals(
							...
							"object",
							this.name +
								".prototype[Symbol.unscopables] should be an object",
						);
						assert_equals(
						...
						assert_equals(
							...
							undefined,
							this.name +
								".prototype should not have @@unscopables",
						);
					}
				}.bind(this),
				this.name +
		...
		IdlInterface.prototype.test_immutable_prototype = function (type, obj) {
			...
			subsetTestByKey(
				...
				((t) => {
					...
					assert_throws_js(globalOf(setter).TypeError, () => {
		...
		IdlInterface.prototype.test_member_const = function (member) {
			...
			subsetTestByKey(
				...
				function () {
					...
					assert_false(
						desc.writable,
						"property should not be writable",
					);
					assert_true(
						desc.enumerable,
			...
			a_test.step(
				function () {
					...
					if (this.is_global()) {
						...
						} else if (member.idlType.generic !== "Promise") {
		...
		IdlInterface.prototype.test_member_operation = function (member) {
			...
			a_test.step(
				function () {
					...
					this.do_member_unscopable_asserts(member);
					this.do_member_operation_asserts(
						memberHolderObject,
						member,
						a_test,
					);
		...
		IdlInterface.prototype.do_member_unscopable_asserts = function (
			member,
		) {
		...
		IdlInterface.prototype.do_member_operation_asserts = function (
			...
			const operationOverloads = this.members.filter(
				(m) =>
					m.type == "operation" &&
					m.name == member.name &&
			...
			var args = member.arguments.map((arg) =>
				create_suitable_object(arg.idlType),
			);
		...
		IdlInterface.prototype.test_to_json_operation = function (
			...
			if (member.has_extended_attribute("Default")) {
				subsetTestByKey(
					...
					this.name +
						" interface: default toJSON operation on " +
						desc,
				);
			} else {
				subsetTestByKey(
		...
		IdlInterface.prototype.test_member_maplike = function (member) {
			subsetTestByKey(
				...
				() => {
					...
					assert_equals(
						iteratorDesc.writable,
						true,
						`@@iterator writable`,
					);

					const sizeDesc = Object.getOwnPropertyDescriptor(
						proto,
						"size",
					);
					assert_equals(
		...
		IdlInterface.prototype.test_object = function (desc) {
			...
			if (this.is_callback()) {
				...
				this.test_primary_interface_of(
					desc,
					obj,
					exception,
					expected_typeof,
				...
				while (current_interface) {
					...
					current_interface =
						this.array.members[current_interface.base];
				}
			}
		};
				...
				if (
					...
					a_test.step(
						function () {
							...
							var args = [];
							var cb = awaitNCallbacks(
								minLength,
								a_test.done.bind(a_test),
							);
		...
		IdlInterface.prototype.has_stringifier = function () {
			...
			if (
				this.members.some((member) => member.special === "stringifier")
			) {
				return true;
			}
		...
		IdlInterface.prototype.do_interface_attribute_asserts = function (
			...
			if (
				...
				assert_equals(
					desc.set.name,
					"set " + member.name,
					"The attribute setter must have the name 'set " +
						member.name +
						"'",
				);
			}
		...
		IdlInterfaceMember.prototype.is_to_json_regular_operation =
			function () {
				return (
					this.type == "operation" &&
					this.special !== "static" &&
			...
			function formatType(type) {
				...
				if (type.generic) {
					result =
						type.generic +
						"<" +
						type.idlType.map(formatType).join(", ") +
						">";
				} else if (type.union) {
					result =
						"(" + type.subtype.map(formatType).join(" or ") + ")";
				} else {
					result = type.idlType;
				}
				if (type.nullable) {
		...
		IdlNamespace.prototype.do_member_operation_asserts = function (
			...
			assert_equals(
				...
				minOverloadLength(
					this.members.filter(
						(m) => m.type == "operation" && m.name == member.name,
					),
				),
			...
			a_test.step(
				function () {
					assert_own_property(
						self[this.name],
						member.name,
						"namespace object missing operation " +
							format_value(member.name),
					);
		...
		IdlNamespace.prototype.test_self = function () {
			...
			subsetTestByKey(
				...
				() => {
					...
					assert_true(
						desc.writable,
						"namespace object should be writable",
					);
					assert_false(
						desc.enumerable,
			...
			subsetTestByKey(
				...
				() => {
					...
					if (this.name === "console") {
						...
						const namespacePrototype =
							Object.getPrototypeOf(namespaceObject);
						assert_equals(
							Reflect.ownKeys(namespacePrototype).length,
							0,
						);
						assert_equals(
