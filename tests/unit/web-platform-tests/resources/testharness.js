...
export default function (self) {
	...
	((global_scope) => {
		...
		function WindowTestEnvironment() {
			...
			this.message_functions = {
				start: [
					...
					(properties) => {
						this._dispatch("start_callback", [properties], {
							type: "start",
				...
				test_state: [
					...
					(test) => {
						this._dispatch("test_state_callback", [test], {
							type: "test_state",
				...
				result: [
					...
					(test) => {
						...
						this._dispatch("result_callback", [test], {
							type: "result",
							test: test.structured_clone(),
						});
					},
				...
				completion: [
					...
					(tests, harness_status, asserts) => {
						var cloned_tests = map(tests, (test) =>
						...
						this._dispatch(
							...
							[tests, harness_status],
							{
								...
								asserts: asserts.map((assert) =>
									assert.structured_clone(),
								),
							},
						);
					},
			...
			on_event(window, "message", (event) => {
				if (
					event.data &&
					event.data.type === "getmessages" &&
					event.source
				) {
					...
					// this environment.
					for (var i = 0; i < this.dispatched_messages.length; ++i) {
						event.source.postMessage(
							this.dispatched_messages[i],
							"*",
						);
					}
				}
		...
		WindowTestEnvironment.prototype._dispatch = function (
			...
			this._forEach_windows((w, same_origin) => {
				if (same_origin) {
					try {
		...
		WindowTestEnvironment.prototype.on_tests_ready = function () {
			...
			add_completion_callback((tests, harness_status, asserts_run) => {
				this.output_handler.show_results(
					tests,
					harness_status,
					asserts_run,
				);
		...
		WindowTestEnvironment.prototype.setup_messages = function (new_events) {
			forEach(settings.message_events, (x) => {
				var current_dispatch = this.message_events.indexOf(x) !== -1;
				var new_dispatch = new_events.indexOf(x) !== -1;
				if (!current_dispatch && new_dispatch) {
					this.message_functions[x][0](this.message_functions[x][2]);
				} else if (current_dispatch && !new_dispatch) {
					this.message_functions[x][1](this.message_functions[x][2]);
				}
			});
		...
		WindowTestEnvironment.prototype.add_on_loaded_callback = (callback) => {
		...
		WorkerTestEnvironment.prototype.on_tests_ready = function () {
			...
			add_completion_callback((tests, harness_status, asserts) => {
				this._dispatch({
					type: "complete",
					tests: map(tests, (test) => test.structured_clone()),
					status: harness_status.structured_clone(),
		...
		function SharedWorkerTestEnvironment() {
			...
			self.addEventListener(
				"connect",
				(message_event) => {
					this._add_message_port(message_event.source);
				},
				false,
			);
		}
		...
		function is_shared_worker(worker) {
			return (
				"SharedWorker" in global_scope && worker instanceof SharedWorker
			);
		}
		...
		function get_test_name(func, name) {
			...
			if (func) {
				...
				if (
					...
					var trimmed = (
						arrow[1] !== undefined ? arrow[1] : arrow[2]
					).trim();
		...
		function async_test(func, name, properties) {
			if (tests.promise_setup_called) {
				...
				tests.status.message =
					"`async_test` invoked after `promise_setup`";
				tests.complete();
			}
		...
		function EventWatcher(test, watchedNode, eventTypes, timeoutPromise) {
			...
			this.wait_for = (types, options) => {
				if (waitingFor) {
					return Promise.reject(
						"Already waiting for an event or events",
					);
				}
			...
			function stop_watching() {
				for (var i = 0; i < eventTypes.length; i++) {
					watchedNode.removeEventListener(
						eventTypes[i],
						eventHandler,
						false,
					);
			...
			switch (typeof val) {
				case "string":
					...
					for (var p in replacements) {
						...
						val = val.replace(
							RegExp(String.fromCharCode(p), "g"),
							replace,
						);
					}
				...
				case "number":
					...
					// special-case.
					if (val === 0 && 1 / val === Number.NEGATIVE_INFINITY) {
						return "-0";
					}
					return String(val);
				case "object":
					...
					if (is_node(val)) {
						switch (val.nodeType) {
							...
							case Node.TEXT_NODE:
								return (
									'Text node "' + truncate(val.data, 60) + '"'
								);
		...
		function assert_array_equals(actual, expected, description) {
			...
			for (var i = 0; i < actual.length; i++) {
				assert(
					...
					{
						i: i,
						expected: expected.hasOwnProperty(i)
							? "present"
							: "missing",
						actual: actual.hasOwnProperty(i)
							? "present"
							: "missing",
		...
		function _assert_inherits(name) {
			return (object, property_name, description) => {
				assert(
		...
		function assert_throws_dom_impl(
			...
			try {
				...
				if (typeof type === "number") {
					...
					name =
						type in codename_name_map
							? codename_name_map[type]
							: type;
		...
		Test.prototype.step_timeout = function (func, timeout) {
			...
			var local_set_timeout =
				typeof global_scope.setTimeout === "undefined"
					...
					: setTimeout;
			return local_set_timeout(
				this.step_func(() => func.apply(this, args)),
				timeout * tests.timeout_multiplier,
			);
		};
		...
		Test.prototype.step_wait_func = function (
			...
			const step = this.step_func((result) => {
				if (result) {
					func();
				} else {
					if (remaining === 0) {
			...
			var wait_for_inner = this.step_func(() => {
				Promise.resolve(cond()).then(
					step,
					this.unreached_func("step_wait_func"),
				);
			});
		...
		Test.prototype.cleanup = function () {
			...
			forEach(this.cleanup_callbacks, (cleanup_callback) => {
				var result;
				...
				if (!is_valid_cleanup_result(this, result)) {
					bad_value_count += 1;
			...
			if (this._is_promise_test) {
				all_async(
					...
					() => {
						cleanup_done(this, errors, bad_value_count);
					},
				);
			} else {
				cleanup_done(this, errors, bad_value_count);
		...
		function cleanup_done(test, errors, bad_value_count) {
			...
			tests.result(test);
			forEach(test._done_callbacks, (callback) => {
				callback();
			});
		...
		Test.prototype.get_signal = function () {
			if (!this._abortController) {
				throw new Error(
					"AbortController is not supported in this browser",
				);
			}
		...
		RemoteTest.prototype.done = function () {
			...
			forEach(this._done_callbacks, (callback) => {
				callback();
			});
		};
		...
		function RemoteContext(remote, message_target, message_filter) {
			...
			this.message_handler = (message) => {
				...
				if (
					...
					message.data &&
					passesFilter &&
					message.data.type in this.message_handlers
				) {
					this.message_handlers[message.data.type].call(
						this,
						message.data,
					);
				}
			...
			this.message_target.addEventListener(
				"message",
		...
		RemoteContext.prototype.report_uncaught = (error) => {
		...
		RemoteContext.prototype.start = function (data) {
			this.started = true;
			this.allow_uncaught_exception =
				data.properties.allow_uncaught_exception;
		...
		RemoteContext.prototype.remote_done = function (data) {
			...
			for (const assert of data.asserts) {
			...
			this.message_target.removeEventListener(
				"message",
				this.message_handler,
			);
			this.running = false;
		...
		function Tests() {
			...
			test_environment.add_on_loaded_callback(() => {
				if (this.all_done()) {
					this.complete();
				}
			});
		...
		Tests.prototype.set_file_is_test = function () {
			if (this.tests.length > 0) {
				throw new Error(
					"Tried to set file as test after creating a test",
				);
			}
			this.wait_for_finish = true;
		...
		Tests.prototype.notify_test_state = function (test) {
			forEach(this.test_state_callbacks, (callback) => {
				callback(test, this);
			});
		};
		...
		Tests.prototype.all_done = function () {
			return (
				...
				!this.pending_remotes.some((w) => w.running)
			);
		};
		...
		Tests.prototype.notify_start = function () {
			forEach(this.start_callbacks, (callback) => {
		...
		Tests.prototype.notify_result = function (test) {
			...
			forEach(this.test_done_callbacks, (callback) => {
				callback(test, this);
			});
			this.processing_callbacks = false;
			if (this.all_done()) {
		...
		Tests.prototype.complete = function () {
			...
			var incomplete = filter(
				this.tests,
				(test) => test.phase < test.phases.COMPLETE,
			);
		...
		Tests.prototype.set_assert_status = function (index, status, stack) {
			const assert_record = this.asserts_run[index];
		...
		Tests.prototype.find_duplicates = function () {
			...
			forEach(this.tests, (test) => {
				if (
					test.name in names &&
					duplicates.indexOf(test.name) === -1
				) {
		...
		Tests.prototype.notify_complete = function () {
			...
			forEach(this.all_done_callbacks, (callback) => {
				callback(this.tests, this.status, this.asserts_run);
			});
		};
		 ...
		 */
		Tests.prototype.create_remote_worker = (worker) => {
			var message_port;
		 ...
		 */
		Tests.prototype.create_remote_window = (remote) => {
			remote.postMessage({ type: "getmessages" }, "*");
			return new RemoteContext(
				remote,
				window,
				(msg) => msg.source === remote,
			);
		};
			...
			if (!node) {
				...
				if (output_document.body) {
					...
					var is_svg =
						...
						root instanceof
							output_document.defaultView.SVGSVGElement;
					if (is_svg) {
		...
		Output.prototype.show_results = function (
			...
			var summary_template = [
				...
				() => {
					...
					while (Test.prototype.status_formats.hasOwnProperty(i)) {
						if (
							status_number.hasOwnProperty(
								Test.prototype.status_formats[i],
							)
						) {
			...
			output_document
				...
				.addEventListener("click", () => {
					const evt = new Event("__test_restart");
					const canceled = !window.dispatchEvent(evt);
					if (!canceled) {
						location.reload();
					}
				});
			...
			forEach(
				...
				(element) => {
					on_event(element, "click", (e) => {
						if (
							output_document.getElementById("results") === null
						) {
							e.preventDefault();
							return;
						}
		...
		function substitute(template, substitutions) {
			...
			return filter(
				map(template, (x) => substitute(x, substitutions)),
				(x) => x !== null,
			);
		}
		...
		function make_dom_single(template, doc) {
			...
			if (template[0] === "{text}") {
				...
				element = output_document.createElementNS(
					xhtml_ns,
					template[0],
				);
				for (var name in template[1]) {
		...
		function make_dom(template, substitutions, output_document) {
			...
			return map(template, (x) => make_dom_single(x, output_document));
		}
		...
		if (global_scope.addEventListener) {
			var error_handler = (error, message, stack) => {
