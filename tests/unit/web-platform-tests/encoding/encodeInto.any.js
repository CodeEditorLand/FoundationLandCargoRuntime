				...
				(arrayBufferOrSharedArrayBuffer) => {
					test(
						() => {
							// Setup
							const bufferLength =
								testData.destinationLength +
								destinationData.bufferIncrease;
							...
							for (let i = 0; i < bufferLength; i++) {
								if (
									...
									assert_equals(
										fullView[i],
										testData.written[i - destinationOffset],
									);
								}
							}
						},
	...
	[
		...
		["ArrayBuffer", "SharedArrayBuffer"].forEach(
			(arrayBufferOrSharedArrayBuffer) => {
				test(
					() => {
						const viewInstance = new self[type](
							createBuffer(arrayBufferOrSharedArrayBuffer, 0),
						);
						assert_throws_js(TypeError, () =>
							new TextEncoder().encodeInto("", viewInstance),
						);
					},
					"Invalid encodeInto() destination: " +
						type +
						", backed by: " +
						arrayBufferOrSharedArrayBuffer,
				);
			...
			test(() => {
				assert_throws_js(TypeError, () =>
					new TextEncoder().encodeInto(
						"",
						createBuffer(arrayBufferOrSharedArrayBuffer, 10),
					),
				);
			}, "Invalid encodeInto() destination: " +
				arrayBufferOrSharedArrayBuffer);
