export default function (self) {
	...
	(() => {
		var subTestStart = 0;
		var subTestEnd = Number.POSITIVE_INFINITY;
		var match;
		if (location.search) {
			match = /(?:^\?|&)(\d+)-(\d+|last)(?:&|$)/.exec(location.search);
			if (match) {
				subTestStart = Number.parseInt(match[1], 10);
				if (match[2] !== "last") {
					subTestEnd = Number.parseInt(match[2], 10);
				}
			}
			...
			if (match) {
				var testsPerVariant = Number.parseInt(match[1], 10);
		...
		function shouldRunSubTest(currentSubTest) {
			return (
				currentSubTest >= subTestStart && currentSubTest <= subTestEnd
			);
		}
