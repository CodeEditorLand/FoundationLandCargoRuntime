...
impl<T:Clone> Sender<T> {
	...
	pub fn subscribe(&self) -> Receiver<T> {
		Receiver { is_sent:self.is_sent.clone(), value:self.value.clone() }
	}
}
...
pub fn channel<T:Clone>() -> (Sender<T>, Receiver<T>) {
	...
	(
		Sender { is_sent:is_sent.clone(), value:value.clone() },
		Receiver { is_sent:is_sent.clone(), value:value.clone() },
	)
}
