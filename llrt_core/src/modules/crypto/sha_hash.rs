...
impl Hmac {
	...
	pub fn new<'js>(ctx:Ctx<'js>, algorithm:String, secret:Value<'js>) -> Result<Self> {
		...
		Ok(Self { context:HmacContext::with_key(&hmac::Key::new(algorithm, &key_value)) })
	}
...
impl Clone for Hmac {
	fn clone(&self) -> Self { Self { context:self.context.clone() } }
}
...
impl Hash {
	...
	pub fn new(ctx:Ctx<'_>, algorithm:String) -> Result<Self> {
		...
		Ok(Self { context:DigestContext::new(algorithm) })
	}
...
impl ShaHash {
	...
	pub fn new<'js>(ctx:Ctx<'js>, algorithm:ShaAlgorithm, secret:Opt<Value<'js>>) -> Result<Self> {
		...
		Ok(ShaHash { secret, bytes:Vec::new(), algorithm })
	}
