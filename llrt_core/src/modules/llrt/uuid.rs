...
fn uuidv1() -> String { Uuid::now_v1(&NODE_ID).format_hyphenated().to_string() }
...
pub fn uuidv4() -> String { Uuid::new_v4().format_hyphenated().to_string() }

fn uuidv6() -> String { Uuid::now_v6(&NODE_ID).format_hyphenated().to_string() }

fn uuidv7() -> String { Uuid::now_v7().format_hyphenated().to_string() }
...
fn stringify<'js>(ctx:Ctx<'js>, value:Value<'js>, offset:Opt<u8>) -> Result<String> {
	let value =
		get_bytes_offset_length(&ctx, value, offset.0.map(|o| o.into()).unwrap_or_default(), None)?;
...
fn validate(value:String) -> bool { Uuid::parse_str(&value).is_ok() }
...
impl From<LlrtUuidModule> for ModuleInfo<LlrtUuidModule> {
	fn from(val:LlrtUuidModule) -> Self { ModuleInfo { name:"llrt:uuid", module:val } }
}
