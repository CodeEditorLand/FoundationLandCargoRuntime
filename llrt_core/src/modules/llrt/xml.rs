...
impl<'js> StackObject<'js> {
	fn new(ctx:Ctx<'js>) -> Result<Self> { Ok(Self { obj:Object::new(ctx)?, has_value:false }) }
...
impl<'js> XMLParser<'js> {
	...
	fn process_tag_value(
		...
		if let Some(tag_value_processor) = &self.tag_value_processor {
			let jpath = path.iter().map(|(k, _)| k.as_ref()).collect::<Vec<_>>().join(".");
...
impl XmlText {
	...
	fn to_string(&self) -> &str { self.value.as_ref() }
}
...
struct XmlNode<'js> {
	#[qjs(skip_trace)]
	name:String,
	// child and attributes are always set to avoid branch checks when
	// adding/removing values
	children:Vec<Value<'js>>,
	#[qjs(skip_trace)]
	// vec iteration is faster since we rarely have more than 10 attrs and we
	// want to retain insertion order
...
impl<'js> XmlNode<'js> {
	...
	fn new(name:String, children:Opt<Vec<Value<'js>>>) -> Result<Self> {
		let node = XmlNode { name, attributes:Vec::new(), children:children.0.unwrap_or_default() };
	...
	fn of(
		...
		let mut node = XmlNode { name, children:Vec::new(), attributes:Vec::new() };
...
impl From<LlrtXmlModule> for ModuleInfo<LlrtXmlModule> {
	fn from(val:LlrtXmlModule) -> Self { ModuleInfo { name:"llrt:xml", module:val } }
}
