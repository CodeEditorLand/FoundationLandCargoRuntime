...
impl<'js> AbortSignal<'js> {
	...
	pub fn new() -> Self {
		...
		Self { emitter:EventEmitter::new(), aborted:false, reason:None, sender }
	}
	...
	pub fn throw_if_aborted(&self, ctx:Ctx<'js>) -> Result<()> {
		if self.aborted {
			return Err(
				ctx.throw(self.reason.clone().unwrap_or_else(|| Undefined.into_value(ctx.clone())))
			);
		}
		Ok(())
	}
	...
	#[qjs(get)]
	pub fn reason(&self) -> Option<Value<'js>> { self.reason.clone() }
