...
impl<'js> Request<'js> {
	...
	#[qjs(get)]
	fn method(&self) -> String { self.method.clone() }

	#[qjs(get)]
	fn headers(&self) -> Option<Class<'js, Headers>> { self.headers.clone() }
	...
	#[qjs(get)]
	fn signal(&self) -> Option<Class<'js, AbortSignal<'js>>> { self.signal.clone() }

	#[qjs(get)]
	fn body_used(&self) -> bool { self.body.is_some() }

	#[qjs(get)]
	fn mode(&self) -> String { "navigate".to_string() }

	#[qjs(get)]
	fn cache(&self) -> String { "no-store".to_string() }
	...
	fn clone(&mut self, ctx:Ctx<'js>) -> Result<Self> {
		let headers = if let Some(headers) = &self.headers {
			Some(Class::<Headers>::instance(ctx.clone(), headers.borrow().clone())?)
		} else {
			None
		};
