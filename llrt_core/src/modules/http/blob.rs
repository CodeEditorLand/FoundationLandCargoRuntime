...
impl Blob {
	...
	pub async fn text(&mut self) -> String { String::from_utf8_lossy(&self.data).to_string() }
...
impl Blob {
	...
	pub fn get_bytes(&self) -> Vec<u8> { self.data.clone() }
...
fn bytes_from_parts<'js>(ctx:&Ctx<'js>, parts:Value<'js>, endings:EndingType) -> Result<Vec<u8>> {
