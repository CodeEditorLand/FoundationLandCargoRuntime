...
impl<'js> Body<'js> {
	...
	pub fn from_value(ctx:&Ctx<'js>, body:Option<Value<'js>>) -> Result<Option<Class<'js, Self>>> {
		if let Some(body) = body {
			...
			return Ok(Some(Class::instance(
				ctx.clone(),
				Self { data:BodyVariant::Provided(body), content_type:None },
			)?));
		}
		Ok(None)
	}
	...
	pub fn from_incoming(
		...
		Class::instance(ctx, Self { data:BodyVariant::Incoming(Some(response)), content_type })
	}
