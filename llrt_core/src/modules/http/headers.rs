...
impl Headers {
	...
	pub fn get_set_cookie(&self) -> Vec<&str> {
		self.headers
			...
			.filter_map(
				|(k, v)| {
					if k.as_ref() == HEADERS_KEY_SET_COOKIE { Some(v.as_ref()) } else { None }
				},
			)
			.collect()
	}
	...
	pub fn keys(&self) -> Vec<&str> { self.headers.iter().map(|(k, _)| k.as_ref()).collect() }

	pub fn values(&self) -> Vec<&str> { self.headers.iter().map(|(_, v)| v.as_ref()).collect() }

	pub fn entries<'js>(&self, ctx:Ctx<'js>) -> Result<Value<'js>> { self.js_iterator(ctx) }

	#[qjs(rename = PredefinedAtom::SymbolIterator)]
	pub fn iterator<'js>(&self, ctx:Ctx<'js>) -> Result<Value<'js>> { self.js_iterator(ctx) }
...
impl Headers {
	...
	pub fn from_http_headers(header_map:&HeaderMap) -> Result<Self> {
		...
		for (n, v) in header_map.iter() {
			headers.push((n.as_str().into(), String::from_utf8_lossy(v.as_bytes()).into()));
		}
...
impl<'js> IteratorDef<'js> for Headers {
	fn js_entries(&self, ctx:Ctx<'js>) -> Result<Array<'js>> {
		map_to_entries(&ctx, self.headers.iter().map(|(k, v)| (k.as_ref(), v.as_ref())))
	}
}
