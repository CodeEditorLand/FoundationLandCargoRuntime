...
use crate::{environment, modules::http::headers::Headers, utils::class::CustomInspectExtension};
...
pub static TLS_CONFIG:Lazy<io::Result<ClientConfig>> = Lazy::new(|| {
	...
	Ok(match env::var(environment::ENV_LLRT_TLS_VERSION).as_deref() {
		Ok("1.3") => builder.with_safe_default_protocol_versions(),
		_ => builder.with_protocol_versions(&[&version::TLS12]), /* Use TLS 1.2 by default to
		                                                          * increase compat and keep
		                                                          * latency low */
	}
	.unwrap()
