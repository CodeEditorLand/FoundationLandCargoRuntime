...
impl<'js> URLSearchParams {
	...
	#[qjs(get)]
	pub fn size(&self) -> usize { self.url.borrow().query_pairs().count() }
	...
	pub fn delete(&mut self, ctx:Ctx<'js>, key:Coerced<String>, value:Opt<Value<'js>>) {
		...
		if !new_pairs.is_empty() {
			self.url.borrow_mut().query_pairs_mut().clear().extend_pairs(new_pairs);
		} else {
			self.url.borrow_mut().set_query(None);
		}
	}

	pub fn entries(&self, ctx:Ctx<'js>) -> Result<Value<'js>> { self.js_iterator(ctx) }
	...
	pub fn get(&mut self, ctx:Ctx<'js>, key:String) -> Result<Value<'js>> {
		match self.url.borrow().query_pairs().find(|(k, _)| *k == key).map(|(_, v)| v) {
	...
	pub fn keys(&mut self) -> Vec<String> {
		self.url.borrow().query_pairs().map(|(k, _)| k.to_string()).collect()
	}
	...
	pub fn set(&mut self, key:Coerced<String>, value:Coerced<String>) {
		...
		self.url.borrow_mut().query_pairs_mut().clear().extend_pairs(new_query_pairs);
	}
	...
	pub fn values(&mut self) -> Vec<String> {
		self.url.borrow().query_pairs().map(|(_, v)| v.to_string()).collect()
	}

	#[qjs(rename = PredefinedAtom::SymbolIterator)]
	pub fn iterator(&self, ctx:Ctx<'js>) -> Result<Class<'js, URLSearchParamsIter>> {
		Class::instance(ctx, URLSearchParamsIter { index:0, params:self.clone() })
	}
}
...
impl<'js> URLSearchParams {
	...
	pub fn from_url(url:&Rc<RefCell<Url>>) -> Self { Self { url:Rc::clone(url) } }
	...
	pub fn from_array(ctx:&Ctx<'js>, array:Array) -> Result<Self> {
		...
		Ok(Self { url:Rc::new(RefCell::new(url)) })
	}
