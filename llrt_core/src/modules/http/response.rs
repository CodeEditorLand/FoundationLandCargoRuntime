...
impl<'js> Response<'js> {
	pub fn from_incoming(
		...
		let body_attributes = BodyAttributes { content_type, content_encoding };
...
impl<'js> Response<'js> {
	...
	#[qjs(get)]
	pub fn status(&self) -> u64 { self.status.into() }

	#[qjs(get)]
	pub fn url(&self) -> String { self.url.clone() }

	#[qjs(get)]
	pub fn ok(&self) -> bool { self.status > 199 && self.status < 300 }
	...
	#[qjs(get)]
	pub fn body(&self) -> Null { Null }

	#[qjs(get)]
	fn headers(&self) -> Class<'js, Headers> { self.headers.clone() }
	...
	fn error(ctx:Ctx<'js>) -> Result<Self> {
		Ok(Self {
			...
			body_attributes:BodyAttributes { content_type:None, content_encoding:None },
			abort_receiver:None,
		})
	}
