...
use crate::{
	...
	modules::{console, http::HTTP_CLIENT},
...
async fn invoke_response<'js>(
	...
	let req = Request::builder()
		...
		.header(CONTENT_TYPE, "application/json")
		.body(Full::from(bytes::Bytes::from(result_json.unwrap_or_default())))
		.or_throw(ctx)?;
...
async fn process_event<'js>(
	...
	let result = match handler_result.as_object() {
		Some(obj) if obj.is_instance_of(promise_constructor) => {
			handler_result.get::<Promise>()?.into_future::<Value>().await?
		},
		_ => handler_result,
	};
...
mod tests {
	...
	async fn runtime() {
		...
		Mock::given(matchers::method("POST"))
			.and(matchers::path_regex(r#"invocation/[A-z0-9-]{1,}/response$"#))
