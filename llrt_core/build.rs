...
async fn main() -> StdResult<(), Box<dyn Error>> {
	...
	let mut sdk_bytecode_file =
		BufWriter::new(File::create(sdk_bytecode_path)?);

	let mut ph_map = phf_codegen::Map::<String>::new();
	let mut lrt_filenames = vec![];
	let mut total_bytes:usize = 0;

	fs::write("../VERSION", env!("CARGO_PKG_VERSION"))
	...
	ctx.with(|ctx| {
		for dir_ent in WalkDir::new(BUNDLE_JS_DIR).into_iter().flatten() {
			...
			let source =
				fs::read_to_string(dir_ent.path()).unwrap_or_else(|_| {
					panic!(
						"Unable to load: {}",
						dir_ent.path().to_string_lossy()
					)
				});
			...
			let lrt_path = PathBuf::from(BUNDLE_LRT_DIR)
				.join(path.with_extension(BYTECODE_EXT));
	...
	write!(
		...
		"// @generated by build.rs\n\npub static BYTECODE_CACHE: \
		 phf::Map<&'static str, &[u8]> = {}",
		ph_map.build()
	)?;
	...
	if cfg!(feature = "uncompressed") {
		generate_compression_dictionary(
			&compression_dictionary_path,
			&lrt_filenames,
		)?;
	} else {
		total_bytes =
			compress_bytecode(compression_dictionary_path, lrt_filenames)?;
...
fn set_nightly_cfg() {
	...
	let version =
		std::process::Command::new(rustc).arg("--version").output().unwrap();
...
fn compress_bytecode(
	...
	for filename in source_files {
		...
		let tmp_filename =
			tmp_dir.join(nanoid::nanoid!()).to_string_lossy().to_string();

		fs::copy(&filename, &tmp_filename)?;

		let uncompressed_file_size =
			PathBuf::from(&filename).metadata()?.len() as u32;
		...
		let compressed =
			add_bytecode_header(bytes, Some(uncompressed_file_size));
		fs::write(&filename, compressed)?;

		let compressed_file_size =
			PathBuf::from(&filename).metadata().unwrap().len() as usize;
...
fn generate_compression_dictionary(
	...
	let mut dictionary_file_set:HashSet<String> =
		HashSet::from_iter(dictionary_filenames.clone());
