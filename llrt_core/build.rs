...
async fn main() -> StdResult<(), Box<dyn Error>> {
	...
	ctx.with(|ctx| {
		for dir_ent in WalkDir::new(BUNDLE_JS_DIR).into_iter().flatten() {
			...
			ph_map.entry(
				module_name,
				&format!("include_bytes!(\"..{}{}\")", MAIN_SEPARATOR_STR, &lrt_filename)
	...
	let compression_dictionary_path =
		Path::new(BUNDLE_LRT_DIR).join("compression.dict").to_string_lossy().to_string();
...
fn compress_bytecode(dictionary_path:String, source_files:Vec<String>) -> io::Result<usize> {
	...
	for filename in source_files {
		...
		let tmp_filename = tmp_dir.join(nanoid::nanoid!()).to_string_lossy().to_string();
		...
		let output = Command::new("zstd")
			.args(["--ultra", "-22", "-f", "-D", &dictionary_path, &tmp_filename, "-o", &filename])
			.output()?;

		if !output.status.success() {
			return Err(io::Error::new(io::ErrorKind::Other, "Failed to compress file"));
		}
