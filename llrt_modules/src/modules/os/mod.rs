...
use crate::{module_info::ModuleInfo, sysinfo::get_platform};
...
fn get_tmp_dir() -> String { env::temp_dir().to_string_lossy().to_string() }
...
impl From<OsModule> for ModuleInfo<OsModule> {
	fn from(val:OsModule) -> Self { ModuleInfo { name:"os", module:val } }
}
...
mod tests {
	...
	async fn test_type() {
		test_async_with(|ctx| {
			Box::pin(async move {
				ModuleEvaluator::eval_rust::<OsModule>(ctx.clone(), "os").await.unwrap();
