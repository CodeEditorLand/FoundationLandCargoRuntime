...
pub fn get_type() -> &'static str { &OS_INFO.0 }

pub fn get_release() -> &'static str { &OS_INFO.1 }

pub fn get_version() -> &'static str { &OS_INFO.2 }
...
fn uname() -> (String, String, String) {
	...
	(
		...
		unsafe { CStr::from_ptr(info.sysname.as_ptr()).to_string_lossy().into_owned() },
		// SAFETY: `info.release` is a valid NUL-terminated pointer.
		unsafe { CStr::from_ptr(info.release.as_ptr()).to_string_lossy().into_owned() },
		// SAFETY: `info.version` is a valid NUL-terminated pointer.
		unsafe { CStr::from_ptr(info.version.as_ptr()).to_string_lossy().into_owned() },
	)
}
