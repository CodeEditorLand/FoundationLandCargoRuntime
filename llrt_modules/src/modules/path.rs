...
pub fn dirname(path:String) -> String {
	...
	match path.rfind(MAIN_SEPARATOR) {
		Some(idx) => {
			let parent = &path[..idx];
			if parent.is_empty() { MAIN_SEPARATOR_STR } else { parent }
		},
		None => ".",
	}
	.to_string()
...
fn join(parts:Rest<String>) -> String { join_path(parts.0) }
...
fn remove_trailing_slash(result:PathBuf) -> String {
	...
	path.strip_suffix(MAIN_SEPARATOR).unwrap_or(&path).to_string()
}

fn resolve(path:Rest<String>) -> String { resolve_path(path.iter()) }
...
pub fn is_absolute(path:String) -> bool { PathBuf::from(path).is_absolute() }
...
impl From<PathModule> for ModuleInfo<PathModule> {
	fn from(val:PathModule) -> Self { ModuleInfo { name:"path", module:val } }
}
