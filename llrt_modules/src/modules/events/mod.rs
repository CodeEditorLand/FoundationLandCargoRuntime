...
use llrt_utils::{error::ErrorExtensions, object::ObjectExt, result::ResultExt};
use rquickjs::{
...
where
	...
	fn on_event_changed(&mut self, _event:EventKey<'js>, _added:bool) -> Result<()> { Ok(()) }
	...
	fn add_event_emitter_prototype(ctx:&Ctx<'js>) -> Result<Object<'js>> {
		...
		proto.set("prependOnceListener", Func::from(Self::prepend_once_listener))?;
	...
	fn add_event_listener(
		...
		let item = EventItem { callback:listener, once };
...
impl From<EventsModule> for ModuleInfo<EventsModule> {
	fn from(val:EventsModule) -> Self { ModuleInfo { name:"events", module:val } }
}
