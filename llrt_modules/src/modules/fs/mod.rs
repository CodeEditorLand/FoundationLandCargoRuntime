...
use self::{
	...
	open::open,
	read_dir::{read_dir, read_dir_sync, Dirent},
...
impl From<FsPromisesModule> for ModuleInfo<FsPromisesModule> {
	fn from(val:FsPromisesModule) -> Self { ModuleInfo { name:"fs/promises", module:val } }
}
...
impl From<FsModule> for ModuleInfo<FsModule> {
	fn from(val:FsModule) -> Self { ModuleInfo { name:"fs", module:val } }
}
