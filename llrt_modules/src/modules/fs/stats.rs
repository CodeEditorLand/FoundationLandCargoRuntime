...
// The Stats implementation is very much based on Unix. The Windows
// implementation tries its best to mimic the implementation of libuv since it
// is the standard. See: https://github.com/libuv/libuv/blob/90648ea3e55125a5a819b32106da6462da310da6/src/win/fs.c
//
// By comparison, the Deno implementation is very basic and doesn't even try
// much. See: https://github.com/denoland/deno/blob/c9da27e147d0681724dd647593abbaa46417feb7/ext/io/fs.rs#L114-L182
//
...
impl Stats {
	#[qjs(skip)]
	pub fn new(metadata:Metadata) -> Self { Self { metadata } }
	...
	pub fn size(&self) -> u64 {
		...
		#[cfg(windows)]
		{
			if self.metadata.is_dir() { 0 } else { self.metadata.file_size() }
		}
	...
	pub fn is_file(&self) -> bool { self.metadata.is_file() }

	/// @deprecated Use `is_directory` instead
	pub fn is_dir(&self) -> bool { self.metadata.is_dir() }

	pub fn is_directory(&self) -> bool { self.metadata.is_dir() }

	/// @deprecated Use `is_symbolic_link` instead
	pub fn is_symlink(&self) -> bool { self.metadata.is_symlink() }

	pub fn is_symbolic_link(&self) -> bool { self.metadata.is_symlink() }
