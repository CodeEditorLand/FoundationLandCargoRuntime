...
use rquickjs::{function::Opt, Ctx, Error, Exception, FromJs, Null, Object, Result, Value};
use tokio::{
	fs::File,
...
use crate::{buffer::Buffer, utils::array_buffer::ArrayBufferView};
...
impl FileHandle {
	pub fn new(file:File, path:PathBuf) -> Self { Self { file:Some(file), path } }
...
impl FileHandle {
	...
	async fn read<'js>(
		...
		let options_1 = match buffer_or_options.0 {
			Some(Either::Left(buffer)) => ReadOptions { buffer:Some(buffer), ..Default::default() },
		...
		let options_2 = match options_or_offset.0 {
			...
			Some(Either::Right(offset)) => {
				ReadOptions { offset:Some(offset), ..Default::default() }
			},
			None => ReadOptions::default(),
		};
		...
		// `File::read` since the read is done in a different thread and we
		// cannot garantee that multiple read calls are not done with the same
		...
		let bytes_read = file.read(&mut buf).await.or_throw_msg(&ctx, "Failed to read file")?;
	...
	async fn sync(&self, ctx:Ctx<'_>) -> Result<()> {
		self.file(&ctx)?.sync_all().await.or_throw_msg(&ctx, "Can't sync file")
	}
	...
	// async fn utimes(&mut self,  ctx: Ctx<'_>, atime: Value<'_>, mtime:
	// Value<'_>) -> Result<()>
		...
		let mut options = match offset_or_options_or_position.0 {
			Some(Either::Left(Either::Left(offset_or_position))) => {
				if buffer_or_string.is_left() {
					WriteOptions { offset:Some(offset_or_position), ..Default::default() }
				} else {
	...
	async fn write_file<'js>(
		...
		file.set_len(0).await.or_throw_msg(&ctx, "Failed to truncate file")?;
