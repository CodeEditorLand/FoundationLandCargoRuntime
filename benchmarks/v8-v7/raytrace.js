...
Flog.RayTracer.Vector.prototype = {
	...
	add: (v, w) => new Flog.RayTracer.Vector(w.x + v.x, w.y + v.y, w.z + v.z),
...
Flog.RayTracer.Material.Chessboard.prototype = Object.extend(
	new Flog.RayTracer.Material.BaseMaterial(),
	{
		...
		getColor: function (u, v) {
			var t =
				this.wrapUp(u * this.density) * this.wrapUp(v * this.density);
		...
		for (var i = 0; i < scene.lights.length; i++) {
			...
			if (depth <= this.options.rayDepth) {
				...
				if (
					...
					var refl = this.testIntersection(
						reflectionRay,
						scene,
						info.shape,
					);
			...
			if (this.options.renderShadows) {
				...
				shadowInfo = this.testIntersection(
					shadowRay,
					scene,
					info.shape,
				);
				if (
					...
					var vA = Flog.RayTracer.Color.prototype.multiplyScalar(
						color,
						0.5,
					);
					var dB =
						0.5 *
						Math.pow(shadowInfo.shape.material.transparency, 0.5);
			...
			if (
				...
				var H = Flog.RayTracer.Vector.prototype
					.subtract(E, Lv)
					.normalize();

				var glossWeight = Math.pow(
					Math.max(info.normal.dot(H), 0),
					shininess,
				);
				color = Flog.RayTracer.Color.prototype.add(
					Flog.RayTracer.Color.prototype.multiplyScalar(
